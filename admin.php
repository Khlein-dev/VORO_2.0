  ...
  <center>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
      if (!empty($_POST['selected'])) {
        ...
        echo "<script>alert('Selected records deleted successfully!'); window.location.href='admin.php';</script>";
        exit;
      } else {
    ...
    <!-- ✅ Log Out -->
    <br><br>
    <a href="index.php" class="btn btn-sm glow" style="width: 100px; position: absolute; right: 20px; top: 20px;">
      ...
      if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['save_admin'])) {
        ...
        if (!empty($adminEmail) && !empty($adminPassword)) {
          ...
          if (mysqli_stmt_num_rows($stmt) > 0) {
            echo "<script>alert('Admin already exists with that email!');</script>";
          } else {
...
<?php else : ?>
  <?php exit("Terminated <a href='login.php' class='btn'>Log In</a>"); ?>
<?php endif; ?>
